...
if(isset($_SESSION["watch_a_save_lang"])){
    ...
    if(file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_nav_". $_SESSION["watch_a_save_lang"] .".php") && file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_search_". $_SESSION["watch_a_save_lang"] .".php") && file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_footer_". $_SESSION["watch_a_save_lang"] .".php")){
        ...
        include_once("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_search_". $_SESSION["watch_a_save_lang"] .".php");
    ...
    //senao
        ...
        include_once("resources/languages/pt-PT/i18n_search_pt-PT.php");
...
<head>
    ...
    <title>
        <?php echo $document_title . (isset($_GET["search"]) && !empty($_GET["search"]) ? "\"" . $_GET["search"] . "\"" : ""); ?>
    </title>
</head>
...
<body>
        ...
        <div class="container-fluid">
            ...
            <div class="collapse navbar-collapse" id="navbarTogglerDemo02">
                ...
                <form method="GET" action="./search.php" class="d-flex">
                    ...
                    <button class="btn btn-primary" type="submit"><?php echo $search_button_content ?></button>
    ...
    <main class="container mt-4 mb-5">
        ...
        <?php
            ...
            $movies = $client->getSearchApi()->searchMovies($_GET["search"]);

            $found_at_least_one = false;

            foreach($movies["results"] as $key => $result){

                if($found_at_least_one == false){

                    echo '<div class="col-12">
                                <h2>'. $movies_search .' ' . (isset($_GET["search"]) && !empty($_GET["search"]) ? "\"" . $_GET["search"] . "\"" : "") . '</h2>
                            </div>
                ...
                echo '<div class="col">
                        ...
                        <a class="no-decoration" href="./movie.php?movieId=' . $result["id"] . '">
                            <div class="card card-height">
            
                                <img src="'. (Empty($result["poster_path"]) ? "./resources/assets/noimage.png" : "https://www.themoviedb.org/t/p/w600_and_h900_bestv2/" . $result["poster_path"]) . '">
            ...
            $tvshows = $client->getSearchApi()->searchTv($_GET["search"]);

            $found_at_least_one = false;

            foreach($tvshows["results"] as $key => $result){

                if($found_at_least_one == false){

                    echo '<div class="col-12  mt-5">
                                <h2>'. $tvshows_search .' ' . (isset($_GET["search"]) && !empty($_GET["search"]) ? "\"" . $_GET["search"] . "\"" : "") . '</h2>
                            </div>
                            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xxl-5 g-3">';
            ...
            $actors = $client->getSearchApi()->searchPersons($_GET["search"]);

            $found_at_least_one = false;

            foreach($actors["results"] as $key => $result){

                if($found_at_least_one == false){

                    echo '<div class="col-12 mt-5">
                                <h2>'. $actors_search .' ' . (isset($_GET["search"]) && !empty($_GET["search"]) ? "\"" . $_GET["search"] . "\"" : "") . '</h2>
                            </div>
                            <div class="row row-cols-1 row-cols-sm-2 row-cols-md-3 row-cols-lg-4 row-cols-xxl-5 g-3">';
                ...
                echo '<div class="col">
                        ...
                        <a class="no-decoration" href="./movie.php?movieId=' . $result["id"] . '">
                            <div class="card card-height">
                                ...
                                <img src="'. (Empty($result["profile_path"]) ? "./resources/assets/noimage.png" : "https://www.themoviedb.org/t/p/w600_and_h900_bestv2/" . $result["profile_path"]) .'">
