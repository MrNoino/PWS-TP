...
include_once("./lib/controller/handle_media_genres.php");

include_once("./lib/controller/handle_movies.php");
...
if(isset($_SESSION["watch_a_save_lang"])){
    ...
    if(file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_nav_". $_SESSION["watch_a_save_lang"] .".php") && file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_new-movie_". $_SESSION["watch_a_save_lang"] .".php") && file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_footer_". $_SESSION["watch_a_save_lang"] .".php")){
        ...
        include_once("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_new-movie_". $_SESSION["watch_a_save_lang"] .".php");
    ...
    //senao
        ...
        include_once("resources/languages/pt-PT/i18n_new-movie_pt-PT.php");
...
if(isset($_POST["add_movie"])){
    ...
    if(isset($_POST["title"], $_POST["release_date"], $_POST["duration"], $_POST["language"]) && !Empty($_POST["title"]) && !Empty($_POST["release_date"]) && !Empty($_POST["duration"]) && !Empty($_POST["language"])){

        $handle_movies = new handle_movies();

        $handle_movies->add_movie($_POST["title"], (isset($_POST["resume"]) && !Empty($_POST["resume"]) ? $_POST["resume"] : NULL), $_POST["release_date"], $_POST["duration"], $_POST["language"], (isset($_POST["genres"]) ? $_POST["genres"] : []), (isset($_FILES["poster"]) && $_FILES["poster"]["size"] > 0 ? $_FILES["poster"] : NULL));

        if($handle_movies->get_code() !== 200){
...
<body>
    ...
    <main class="container mt-5 mb-5">
        ...
        <div class="row justify-content-center">
            ...
            <h2 class="text-center text-primary"><?php echo $new_movie_label ?></h2>

        </div>

        <div class="row justify-content-center mt-5">

            <form method="POST" class="col-12 col-md-6 col-xl-4" enctype="multipart/form-data">

                <label for="input-title" class="form-label"><?php echo $title_label ?><span class="text-danger">*</span></label>
                    
                <input type="text" class="form-control<?php echo (isset($error) ? " is-invalid" : ""); ?>" id="input-title" name="title" placeholder="<?php echo $title_placeholder ?>" required/>

                <label for="input-resume" class="form-label mt-4"><?php echo $resume_label ?></label>
                    
                <textarea class="form-control mh-25" id="input-resume" name="resume" max="2048" placeholder="<?php echo $resume_placeholder ?>"></textarea>

                <label for="input-release-date" class="form-label mt-4"><?php echo $release_date_label ?><span class="text-danger">*</span></label>
                    
                <input type="date" class="form-control<?php echo (isset($error) ? " is-invalid" : ""); ?>" id="input-release-date" name="release_date" required/>

                <label for="input-duration" class="form-label mt-4"><?php echo $duration_label ?><span class="text-danger">*</span></label>
                    
                <input type="number" class="form-control<?php echo (isset($error) ? " is-invalid" : ""); ?>" id="input-duration" name="duration" step="1" placeholder="<?php echo $duration_placeholder ?>" required/>

                <label for="movie-language-select" class="form-label mt-4"><?php echo $movie_language_label ?><span class="text-danger">*</span></label>
                    
                <select class="form-select<?php echo (isset($error) ? " is-invalid" : ""); ?>" id="movie-language-select" name="language" required>

                <?php

                    if(!isset($handle_movies)){
                    
                        $handle_movies = new handle_movies();

                    }

                    foreach($handle_movies->get_all_languages() as $language){
                ...
                <label for="genders-select" class="form-label mt-4"><?php echo $genders_label ?></label>
                    
                <select class="form-control" id="genres-select" name="genres[]" multiple>

                    <?php

                        $handle_media_genres = new handle_media_genres();

                        foreach($handle_media_genres->get_all_genres() as $genre){
                ...
                <label for="input-poster" class="form-label mt-4"><?php echo $poster_label ?></label>
                    
                <input type="file" class="form-control" id="input-poster" name="poster" accept="image/*"/>
                ...
                <input type="submit" class="btn btn-primary w-100 mt-5" name="add_movie" value="<?php echo $add_movie_label ?>"/> 
