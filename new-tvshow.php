...
##############################################
##                                          ##
## Título: Watch a Save - New Tv Show       ##
##                                          ##
##############################################
##                                          ##
...
include_once("lib/controller/handle_tvshows.php");
...
if(isset($_SESSION["watch_a_save_lang"])){
    ...
    if(file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_nav_". $_SESSION["watch_a_save_lang"] .".php") && file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_new-tvshow_". $_SESSION["watch_a_save_lang"] .".php") && file_exists("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_footer_". $_SESSION["watch_a_save_lang"] .".php")){
        ...
        include_once("resources/languages/". $_SESSION["watch_a_save_lang"] ."/i18n_new-tvshow_". $_SESSION["watch_a_save_lang"] .".php");
    ...
    //senao
        ...
        include_once("resources/languages/pt-PT/i18n_new-tvshow_pt-PT.php");
...
if(isset($_POST["add_tvshow"])){
    ...
    if(isset($_POST["name"], $_POST["release_date"], $_POST["language"]) && !Empty($_POST["name"]) && !Empty($_POST["release_date"]) && !Empty($_POST["language"])){

        $handle_tvshows = new handle_tvshows();

        $handle_tvshows->add_tvshow($_POST["name"], (isset($_POST["resume"]) && !Empty($_POST["resume"]) ? $_POST["resume"] : NULL), $_POST["release_date"], $_POST["language"], (isset($_POST["genres"]) ? $_POST["genres"] : []), (isset($_FILES["poster"]) && $_FILES["poster"]["size"] > 0 ? $_FILES["poster"] : NULL));

        if($handle_tvshows->get_code() !== 200){
...
<body>
    ...
    <main class="container mt-4 mb-5">
        ...
        <div class="row justify-content-center">
            ...
            <h2 class="text-center text-primary"><?php echo $new_tvshow_label ?></h2>
        ...
        <div class="row justify-content-center mt-5">
            ...
            <form method="POST" class="col-12 col-md-6 col-xl-4" enctype="multipart/form-data">
                ...
                <label for="input-name" class="form-label"><?php echo $name_label ?><span class="text-danger">*</span></label>
                    
                <input type="text" class="form-control<?php echo (isset($error) ? " is-invalid" : ""); ?>" id="input-name" name="name" placeholder="<?php echo $name_placeholder ?>" required/>
                ...
                <label for="tvshow-language-select" class="form-label mt-4"><?php echo $tvshow_language_label ?><span class="text-danger">*</span></label>
                    
                <select class="form-select<?php echo (isset($error) ? " is-invalid" : ""); ?>" id="tvshow-language-select" name="language" required>

                <?php

                    if(!isset($handle_tvshows)){
                    
                        $handle_tvshows = new handle_tvshows();

                    }

                    foreach($handle_tvshows->get_all_languages() as $language){
                ...
                <label for="genres-select" class="form-label mt-4"><?php echo $genres_label ?></label>
                ...
                <input type="submit" class="btn btn-primary w-100 mt-5" name="add_tvshow" value="<?php echo $add_tvshow_label ?>"/> 
